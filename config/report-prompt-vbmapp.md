...
1. **得分计算**：每个里程碑 0/0.5/1 分，每领域每阶段 5 个里程碑，满分 5 分
2. **里程碑网格填色**：得分 1=全填色(cell-filled)，0.5=半填色(cell-half)，0=空(cell-empty)
3. **领域得分率** = 领域总得分 / 领域满分 × 100%
4. **领域评级**（基于得分率，确定性映射）：
   - 得分率 ≥ 70% → "表现良好"
   - 得分率 40%~69% → "发展中"
   - 得分率 < 40% → "需重点关注"
5. **发展阶段判定**（基于里程碑总分）：
   - 总分 0~45 → "相当于正常孩子 0~18 个月（第一阶段）的水平"
   - 总分 46~90 → "相当于正常孩子 18~30 个月（第二阶段）的水平"
   - 总分 91~170 → "相当于正常孩子 30~48 个月（第三阶段）的水平"
6. **优势领域**：得分率 ≥ 70% 的领域
7. **薄弱领域**：得分率 < 40% 的领域
8. `domainReports` 只需为得分 >0 的领域生成报告，得分为 0 的领域标注"尚未起步"

### 障碍评估量化规则

1. **障碍严重度**（基于单项得分，确定性映射）：
   - 0 分 → "无明显障碍"
   - 1 分 → "轻微障碍，影响较小"
   - 2 分 → "中等障碍，需关注"
   - 3 分 → "较严重障碍，需优先干预"
...
2. **障碍总体评级**（基于总分/96）：
   - 总分 0~24 → "整体障碍水平较低"
   - 总分 25~48 → "存在一定障碍，需在教学中注意"
   - 总分 49~72 → "障碍较为显著，需制定针对性干预"
   - 总分 73~96 → "障碍严重，需优先全面干预"
3. **topConcerns**：得分 ≥ 3 的障碍项，按得分降序排列
4. `items` 中优先列出得分 ≥ 2 的障碍

### 转衔评估量化规则

1. **单项评级**（基于得分/5）：
   - 0~1 分 → "需重点关注，尚不具备该领域的转衔准备"
   - 2~3 分 → "需进一步提升，部分具备转衔准备"
   - 4~5 分 → "表现良好，已具备该领域的转衔准备"
2. **转衔总体建议**（基于总分/90）：
   - 总分 0~30 → "建议以密集个训为主，暂不建议融合环境"
   - 总分 31~60 → "建议个训为主、小组为辅，逐步增加集体活动"
   - 总分 61~90 → "可考虑融合教育环境，配合适当支持"
3. 按三类分组展示，每类计算小计

### 综合分析量化规则

1. `analysis.overview`：必须包含三大模块的总分和发展阶段判定
2. `analysis.strengths`：列出得分率 ≥ 70% 的里程碑领域 + 得分 ≥ 4 的转衔项
3. `analysis.concerns`：列出得分率 < 40% 的里程碑领域 + 得分 ≥ 3 的障碍项
4. `analysis.recommendations`：基于以下优先级排序：
   - P1：得分 ≥ 3 的障碍项（优先消除学习障碍）
   - P2：得分率 < 40% 的里程碑领域（优先提升薄弱技能）
   - P3：得分 ≤ 1 的转衔项（提升转衔准备）
5. 所有文本使用中文，ABA 专业术语保持准确（如提要求=Mand，命名=Tact 等）
6. **确定性要求**：同一份输入数据，必须生成完全相同的报告内容。不使用随机措辞，不使用"可能""大概"等模糊表述，所有判断基于上述量化阈值
