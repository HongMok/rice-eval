...
- 默认只读展示：头像（32px 圆形）+ 名称 + 类型 + 描述 + 评估工具介绍状态（"已配置"或灰色斜体"未配置"），右上角"编辑"链接（紫色）
- 点击"编辑"弹出右侧抽屉（宽度 520px），包含：
  ...
  - 评估工具介绍（富文本编辑器）：
    - 使用 contenteditable div 实现（id="toolIntroEditor"），不引入第三方库
    - 工具栏按钮：加粗(B)、斜体(I)、下划线(U)、标题(H)、有序列表(OL)、无序列表(UL)
    - 工具栏样式：灰色背景条（#F3F4F6），按钮 28x28px，hover 高亮（#E5E7EB）
    - 编辑区域：min-height 200px，border 1px solid #D1D5DB，border-radius 8px，padding 12px
    - 数据存储：工具对象新增 `intro` 字段（HTML 字符串），保存时读取 innerHTML，加载时设置 innerHTML
...
2. THE 基本信息栏 SHALL 默认只读展示：头像（32px 圆形）+ "名称: XXX" + "类型: XXX" + "描述: XXX" + "评估工具介绍: 已配置/未配置"，右上角"编辑"链接
3. WHEN 管理员点击"编辑"链接, THE 编辑页 SHALL 弹出右侧抽屉（520px），带遮罩层，包含工具图标上传、名称输入、类型（只读）、描述 textarea、评估工具介绍富文本编辑器
...
8. WHEN 管理员在抽屉中修改信息并点击"保存", THE 系统 SHALL 保存修改（包括评估工具介绍的 HTML 内容）并更新只读展示区的内容
9. THE 评估工具介绍富文本编辑器 SHALL 使用 contenteditable div（id="toolIntroEditor"）实现，工具栏包含加粗(B)、斜体(I)、下划线(U)、标题(H)、有序列表(OL)、无序列表(UL)按钮，编辑区 min-height 200px
10. THE 工具对象数据结构 SHALL 包含 `intro` 字段（HTML 字符串），保存时从 toolIntroEditor 读取 innerHTML，加载时设置 innerHTML
